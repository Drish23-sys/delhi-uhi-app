...
def compute_features(bands):
    ...
    for arr in [NDVI, NDBI, BUFrac, Albedo]:
        for w in WINDOWS:
            spatial.append(uniform_filter(arr, w))
    ...
    for arr in [NDVI, BUFrac]:
        for w in [7, 15, 21]:
            tex.append(local_std(arr, w))

    diff = []
    for arr in [NDBI, BUFrac, NDVI]:
        for wa, wb in [(3,7),(7,21),(21,31)]:
            diff.append(uniform_filter(arr, wa) - uniform_filter(arr, wb))

    all_feats = raw + spatial + tex + diff
    H, W = b2.shape
...
def read_geotiff(path):
    try:
        ...
        with rasterio.open(path) as src:
            n = src.count
            if n < 7:
                return None, f"Expected ≥7 bands, found {n}.", None, None
            data   = src.read()
            bounds = src.bounds
        return data, None, None, bounds
    except Exception as e:
        return None, str(e), None, None

def make_map(label_map):
...
st.markdown(
    ...
    padding:12px 16px; border-radius:4px; font-size:14px; color:#1e3a5f;'>
    Upload a <b>Landsat 9 GeoTIFF</b> (7 bands: B2–B7 + B10).
    ...
    <b style='color:#EF4444'>Hot</b> using XGBoost (94.01% accuracy).
    </div>""",
    unsafe_allow_html=True,
)
st.markdown("---")
...
with col_left:
    ...
    uploaded = st.file_uploader(
        ...
        type=["tif", "tiff"],
        help="Multi-band GeoTIFF — bands in order: B2, B3, B4, B5, B6, B7, B10"
    )
    st.markdown("#### Expected Band Order")
    ...
    if uploaded is None:
        ...
        with st.spinner("Reading GeoTIFF…"):
            with tempfile.NamedTemporaryFile(suffix=".tif", delete=False) as tmp:
                ...
                tmp_path = tmp.name
            data, err, _, bounds = read_geotiff(tmp_path)
            os.unlink(tmp_path)
            if err:
                st.error(f"❌ {err}")
                st.stop()
        ...
        st.success(f"✅ Loaded — {W}×{H} pixels, {data.shape[0]} bands")
        ...
        st.success(f"✅ Features ready — {valid.sum():,} valid pixels")
        ...
        m1.markdown(f"<div class='metric-card'><div class='metric-val'>{total:,}</div>"
                    f"<div class='metric-lbl'>Valid Pixels</div></div>", unsafe_allow_html=True)
        m2.markdown(f"<div class='metric-card'><div class='metric-val cool-val'>"
                    f"{n_cool/total*100:.1f}%</div><div class='metric-lbl'>Cool</div></div>",
                    unsafe_allow_html=True)
        m3.markdown(f"<div class='metric-card'><div class='metric-val mod-val'>"
                    f"{n_mod/total*100:.1f}%</div><div class='metric-lbl'>Moderate</div></div>",
                    unsafe_allow_html=True)
        m4.markdown(f"<div class='metric-card'><div class='metric-val hot-val'>"
                    f"{n_hot/total*100:.1f}%</div><div class='metric-lbl'>Hot</div></div>",
                    unsafe_allow_html=True)

        st.markdown("#### UHI Classification Map")
        fig = make_map(label_map)
        st.pyplot(fig)
        plt.close(fig)
        ...
        cmap2   = ListedColormap(["#050A14", "#2196F3", "#FFC107", "#F44336"])
